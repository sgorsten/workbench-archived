...
size_t glyph_map::insert_glyph(glyph_data glyph)
{
    const size_t index = glyphs.size();
    glyphs.push_back(glyph);
    return index;
}

void glyph_map::prepare_texture()
{
    // Sort glyphs by descending height, then descending width
    std::vector<glyph_data *> glyph_list;
    for(auto & g : glyphs) glyph_list.push_back(&g);
...
const glyph_data * font::get_glyph(int codepoint) const
{
    ...
    return &map.get_glyph(it->second);
}
...
{         
    ...
    for(auto ch : text)
    {
        ...
        width += map.get_glyph(it->second).advance;
    }
    return width;
}
...
std::string::size_type font::get_cursor_pos(const std::string & text, int x) const
{
    for(std::string::size_type i=0; i<text.size(); ++i)
    {
        ...
        if(it == end(glyphs)) continue;
        if(x*2 < map.get_glyph(it->second).advance) return i;
        x -= map.get_glyph(it->second).advance;
    }
    return text.size();
}
...
void font::load_glyphs(const std::string & path, int size, const std::vector<int> & codepoints)
{
    ...
    for(auto codepoint : codepoints)
    {
        ...
        glyphs[codepoint] = map.insert_glyph(data);
    }
}
...
void font::make_circle_quadrant(int radius)
{
    ...
    data.advance = 0;
    glyphs[-radius] = map.insert_glyph(data);
}